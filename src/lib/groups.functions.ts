...
export type GroupPost = {
  ...
  user_id: string;
  author: string | null;
  replies: { id: string; body: string; created_at: string; user_id: string; author: string | null }[];
};
...
export const listGroups = createServerFn({ method: "GET" })
  ...
  .handler(
    async ({
      context,
    }): Promise<{ groups: GroupSummary[]; myGroupIds: string[]; counts: Record<string, number> }> => {
...
export const createGroup = createServerFn({ method: "POST" })
  ...
  .inputValidator((input: { name: string; description: string; is_public: boolean }) => {
    ...
    return {
      ...
      description: String(input.description ?? "").trim().slice(0, 600) || null,
  ...
  .handler(
    async ({
      ...
      const { data: group } = await context.supabase
        ...
        .eq("id", data.groupId)
        .maybeSingle();
      if (!group) return { group: null, isMember: false, isOwner: false, members: [], leaders: [], posts: [] };
      ...
      const [{ data: profiles }, { data: board }, { data: posts }, { data: replies }] = await Promise.all([
        context.supabase.from("profiles").select("id, full_name, avatar_url, points").in("id", ids),
        context.supabase
          .from("leaderboard")
